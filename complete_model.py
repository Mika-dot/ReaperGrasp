...
class CompleteDefectDetector(nn.Module):
    def __init__(self, yolo_path=None, ae_path=None, device=None):
        super().__init__()
        device = device if device is not None else Config.DEVICE

        self.yolo_backbone = YoloBackboneExtractor(yolo_path or Config.YOLO_PATH)
        self.autoencoder = Autoencoder(latent_dim=Config.AE_EMB_SIZE)

        ae_checkpoint = torch.load(ae_path or Config.AE_PATH, map_location='cpu', weights_only=True)
        ae_state = (
        ...
        # Retained only to read legacy checkpoints. Neither branch has a supervised loss.
        for p in self.yolo_head.parameters(): p.requires_grad = False
        for p in self.classifier.embedding_proj.parameters(): p.requires_grad = False
        self.to(device)
        print(f"Complete model создана на {device}")
    ...
    def forward(self, frame_sequence: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Legacy contextual detection head was never trained: do not compute fictitious boxes.
    ...
    def infer_frame(self, frame_t: torch.Tensor, history_buffer: List[torch.Tensor]):
        ...
        ready = len(history_buffer) == Config.SEQ_LENGTH
        if ready:
        ...
        return {
            ...
            "class_name": Config.CLASS_NAMES[pred_class] if ready else "warming_up",
            "ready": ready,
            "ae_error": float(ae_error.item()),
            "history_size": len(history_buffer),
            "class_probabilities": class_probs.detach().cpu().tolist() if ready else None,
            "attention_weights": (
