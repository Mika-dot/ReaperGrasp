...
- `evidence/audit.json` — загрузка весов, градиенты, контроль пересечений.
- `evidence/original_replay.json` — исходное поведение двух клипов.
- `evidence/replay/frames.json` и `summary.json` — исправленный прогон.
- `evidence/ui_smoke.json` — страницы, два потока, запись событий, изображения, остановка и отсутствующий источник.
- `evidence/tests.txt` — результаты 7 автоматических проверок.
